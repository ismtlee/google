...
function all_tokens() {
       $sql = "select pub_id, tokens from accounts where enable = 1 and type = 1";
  ...
  $opt_params = array(
     ...
     'metric' => array('earnings', 
		       ...
		       'individual_ad_impressions_rpm',
                       ...
                       'dimension' => array('date', 'ad_unit_name', 'APP_NAME', 'APP_ID', 'AD_CLIENT_ID')
        );
...
function save_report($rp, $pub_id) {
    ...
    $conn = JoyDb::getInstance();
    foreach ($rp as $e) {
      $sql = "replace into rp_admob(ad_unit_name, app_name, app_id, ad_client_id, 
  ...
  individual_ad_impressions, page_views_ctr,page_views_rpm, page_views) values(?, ?, ?, ?, ?, ?, ?,?,?,?,?,?,?)";
      $stmt = $conn->prepare($sql);
      $params = array($e[1], $e[2], $e[3], $e[4], $e[6], $e[5], $e[0], $e[7], $e[8], $e[9], $e[10], $e[11], $e[12]);
